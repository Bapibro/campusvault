...
export default function PricingCard({
  ...
  return (
    <motion.div
      className={`card-border relative overflow-hidden rounded-[2rem] border p-8 shadow-card bg-white/5 transition ${featured ? 'border-purple-400/30 bg-purple-500/10' : 'border-slate-700/70'}`}
      whileHover={{ y: -8, scale: 1.01, boxShadow: '0 0 40px rgba(139, 92, 246, 0.22)' }}
      transition={{ duration: 0.35, ease: 'easeOut' }}
    >
      <div className="absolute inset-x-0 top-0 h-24 bg-gradient-to-r from-violet-500/10 to-fuchsia-500/5" />
